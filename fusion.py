 # import numpy as np import matplotlib.pyplot as plt r_ball = 0.10
 # 10 cm radius (full diameter 20 cm) faces = 12 np.ones(12) + 20 np.ones(20)
 # pentagons + hexagons coil_arc = np.pi / 12
 ...
 # plot plt.figure(figsize=(8,8)) plt.plot(coil_x, coil_y, ‘c-’, lw=4, label=‘Coils’) plt.plot(x_plasma, y_plasma, ‘r-’, lw=3, label=‘Plasma’) plt.plot(field_x, field_y, ‘b-’, alpha=0.6, label=‘B-field’) plt.axis(‘equal’) plt.title(‘Soccer Ball Reactor - 10 kW @ 18000 RPM’) plt.legend() plt.show()
 # mass_flpow = 2.2
 # kg/s supercritical co2 cp = 850
 # j/kg-k (supercritical) Tin = 193
 # K max allowed (-78 c) power_waste = 10e3
 #
